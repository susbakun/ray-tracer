...
use crate::{
    ...
    model::Model,
    prelude::*,
    shape::{ConstantMedium, Quad, Sphere, Triangle, create_box},
    texture::{CheckerTexture, ImageTexture, NoiseTexture},
    transformation::*,
    vector::{Point3, Vec3},
};

pub fn bouncing_sphere() -> Result<()> {
    let mut rng = rng();
...
pub fn final_scene(image_width: u64, samples_per_pixel: u64, max_depth: u64) -> Result<()> {
...
pub fn teapot_scene() -> Result<()> {
    ...
    let model = Arc::new(Scale::new(Arc::new(model), 3.0));
    ...
    camera.vfov = 80.0;
    camera.lookfrom = Point3::new(0.0, 0.0, 15.0);
